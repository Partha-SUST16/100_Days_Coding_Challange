...
public:
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        for(int i = 0 ; i < inorder.size() ; i++)
            mp[inorder[i]] = i;
        return ConstructTreeFromInPost(inorder,postorder,0,inorder.size()-1,postorder.size()-1);
    }

    TreeNode* ConstructTreeFromInPost(vector<int>& inorder, vector<int>& postorder,int start,int end,int index)
    {
